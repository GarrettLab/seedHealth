...
#' @param vwxtnormm mean of truncated normal distribution of weather parameter (8-27)
#' @param vwxtnormsd sd of truncated normal distribution of weather parameter (8-27)
#' @param vwxd set of potential weather variables from which one is drawn in each time step (8-27)
#' @param vwdsamp if T, a set of potential weather variables is read (wxd); if F, defaults to reading a mean and variance for environmental effects (wxtnormm and extnormsd)(8-27)
...
mpc <- function(vpHSinit=c(0.2,0.5,0.8), vKx = 100, vbetax=c(0.02,0.04), vwxtnormm=c(0.3,0.7), vwxtnormsd=c(0.3,  0.1), vwxd=NULL, vwdsamp=NULL, vhx=1, vmxtnormm=1, vmxtnormsd=0.1, vaxtnormm=c(1,0.5), vaxtnormsd= c(0.3, 0.1), vrx=c(0.1,0.3), vzxtnormm=c(1,0.5), vzxtnormsd= c(0.3, 0.1), vgx=4, vcx=0.9, vphix=c(0,0.5), vnseasons=10, vnsim=1, vHPcut=0.5, vpHScut=0.5,vmaY=100, vmiY=0,  vthetax=c(-0.5,0,0.5), vEx=0.02){

ncomb <- length(vpHSinit) * length(vKx) * length(vbetax) * length(vhx) * length(vmxtnormm) *length(vmxtnormsd) * length(vaxtnormm) * length(vaxtnormsd) * length(vrx) * length(vzxtnormm) * length(vzxtnormsd) * length(vgx) * length(vcx) * length(vphix) * length(vnseasons) * length(vnsim)* length(vHPcut)* length(vpHScut)* length(vmaY)* length(vmiY)* length(vthetax)* length(vEx)

# there may be more combinations if the weather parameters are generated from a truncated normal distribution (vwdsamp=F)

if(!vwdsamp){ #modified 2018-08-27
  ncomb <- ncomb*length(vwxtnormm) * length(vwxtnormsd)
}

if(vwdsamp){ #modified 2018-08-27
  vwxtnormm <- -888 # temporary fix - indicator for output
  vwxtnormsd <- -888 # temporary fix - indicates vwdsamp = T
}
...
# the parameters to track in the output

pars1 <- c('pHSinit', 'Kx', 'betax', 'wxtnormm', 'wxtnormsd', 'hx', 'mxtnormm', 'mxtnormsd', 'axtnormm', 'axtnormsd', 'rx', 'zxtnormm', 'zxtnormsd', 'gx', 'cx', 'phix', 'nseasons', 'nsim', 'HPcut', 'pHScut', 'maY', 'miY', 'thetax', 'Ex')


# prepare a data frame to hold the output, with columns representing the parameters, the mean of the state variables, the median of the state variables, the variance of the state variables, 0.05, and 0.95

# to be removed - note that previously the number of columns was fixed at 84, should be calculated 

num.columns <- length(basn)*5 + length(pars1)

# for dimnames, note that the first entry in the list labels the rows 1:ncomb, and the other entries label the columns

outmpc <- as.data.frame(matrix(data=-999, nrow=ncomb, ncol=num.columns, dimnames = list(1:ncomb, c(pars1, paste(basn,'mean',sep=''), paste(basn,'median',sep=''), paste(basn,'var',sep=''), paste(basn,'q0.05',sep=''), paste(basn,'q0.95',sep='')))))
