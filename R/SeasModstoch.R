...
#' @param wx environmental effect on transmission rate (old - to be removed from this function)
#' @param wxtnormm mean of truncated normal distribution of weather parameter (8-27)
#' @param wxtnormsd sd of truncated normal distribution of weather parameter (8-27)
#' @param wxd set of potential weather variables from which one is drawn in each time step (8-27)
#' @param wdsamp if T, a set of potential weather variables is read (wxd); if F, defaults to reading a mean and variance for environmental effects (wxtnormm and extnormsd)(8-27)
...
SeasModstoch <- function(pHSinit=0.8, Kx = 100, betax=0.02, wxtnormm=NULL, wxtnormsd=NULL, wxd=NULL, wdsamp=NULL, hx=1, mxtnormm=1, mxtnormsd=0.1, axtnormm=1, axtnormsd=0.1, rx=0.1, zxtnormm=1, zxtnormsd= 0.1, gx=4, cx=0.9, phix=0, nseasons=10, HPcut=0.5, pHScut=0.5, maY=100, miY=0, thetax=0.2,  Ex=0) {
...
# 18-08-27 add option to draw from distribution

   outm$wx[si] <- ifelse(wdsamp, sample(wxd,size=1), ifelse(wxtnormsd==0, wxtnormm, (altrtruncnorm(1,a=0,b=1,mean=wxtnormm,sd=wxtnormsd)))) 
